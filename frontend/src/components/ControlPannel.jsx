...
const EventControlPanel = () => {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      try {
        // You'll need a backend route for: GET /api/events/:id
        const res = await axios.get(`http://localhost:5000/api/events/${id}`);
        setEventData(res.data);
        
        // Fetch Attendees (Requirement #3)
...
const StatBox = ({ label, value, change }) => (
  <div className="bg-gray-900/30 border border-gray-800 p-5 rounded-2xl">
    <p className="text-[9px] font-mono text-gray-500 uppercase tracking-widest mb-2">{label}</p>
    <div className="flex items-end justify-between">
      <p className="text-2xl font-black tracking-tighter">{value}</p>
      <span className="text-[10px] font-mono text-green-500 uppercase">{change}</span>
    </div>
  </div>
);
